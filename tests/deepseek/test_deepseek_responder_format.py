"""
DeepSeek 响应格式转换测试 - Responder 的 OpenAI 格式转换验证
...
class TestDeepSeekResponderNonStreamFormat:
    """DeepSeek 非流式响应转换测试"""
    ...
    def test_basic_format_conversion(self, responder, sample_raw_response):
        """验证基本格式转换"""
        result = responder.to_openai_format(sample_raw_response, "deepseek-chat")
...
class TestDeepSeekResponderStreamFormat:
    """DeepSeek 流式响应转换测试"""
    ...
    def test_stream_chunk_format(self, responder, stream_chunk):
        """验证流式 chunk 格式"""
        result = responder.to_openai_stream_format(stream_chunk, "deepseek-chat")
