"""
CNLLM 响应 YAML 配置测试 - response 配置结构与适配器注册验证
...
class TestResponseYAML:
    ...
    def test_fields_config(self, response_config):
        ...
        assert "usage" in fields or ("prompt_tokens" in fields and "completion_tokens" in fields), \
            "fields 应包含 usage 相关映射"
...
class TestVendorAdapterRegistration:
    """厂商适配器注册测试"""

    def test_xiaomi_adapter_registered(self):
        """验证 Xiaomi 适配器已注册"""
        from cnllm.core.adapter import BaseAdapter

        names = BaseAdapter.get_all_adapter_names()
        assert "xiaomi" in names, f"xiaomi 应已注册，实际注册列表: {names}"

        print(f"\n[PASS] Xiaomi 适配器已注册")
        print(f"  已注册适配器: {names}")

    def test_xiaomi_adapter_can_be_retrieved(self):
        """验证可以通过名称获取 Xiaomi 适配器"""
        from cnllm.core.adapter import BaseAdapter

        adapter_class = BaseAdapter.get_adapter_class("xiaomi")
        assert adapter_class is not None, "应能通过名称获取 xiaomi 适配器类"
        assert adapter_class.__name__ == "XiaomiAdapter", \
            f"获取的应是 XiaomiAdapter，实际: {adapter_class.__name__}"

        print(f"\n[PASS] Xiaomi 适配器可正常获取")
