...
class StepVendorError(VendorError):
    VENDOR_NAME = "step"

    @classmethod
    def from_response(cls, raw_response: dict) -> Optional["StepVendorError"]:
        if not raw_response:
            return None
...
VendorErrorRegistry.register("step", StepVendorError)


class StepResponder(Responder):
    CONFIG_DIR = "step"
