...
中文大模型的能力已跻身第一梯队，但在实际的生产环境中却缺少足够的基础设施。一个无法忽视的、**两难的痛点**在于：
使用 OpenAI SDK 和厂商提供的兼容接口时，部分模型**原生参数被静默忽略**，导致**结果不可控以及功能缺失**；而使用厂商自研 SDK 则需要进行**额外的字段解析、结构转换**，当工作流涉及使用不同厂商的多个模型，则需要为不同模型做不同的代码适配，导致**实现、维护成本上升**。

CNLLM 提供了一个**统一的 OpenAI 兼容接口层**与一套**标准化的参数规则和响应格式规范**。通过标准化、可配置的 YAML 文件实现请求端和响应端的双重映射，将异构的模型原生响应自动封装为 OpenAI 标准响应，同时完整保留中文大模型的原生参数和能力。
...
- 🔧 **流式累积修复** 
  - 修复 StreamAccumulator 重复包装导致的 `.still`/`.think` 重复累积问题
- ✨ **reasoning_content 标准响应** 
  - `reasoning_content` 现在包含在 OpenAI 标准响应体的 `choices[0].message` 中，通过 `.think` 访问
- ✨ **reasoning_content 流式 delta** 
  - 流式 chunk 的 `choices[0].delta` 中现在包含 `reasoning_content` 字段，通过 `.think` 访问，返回实时累积的推理内容
...
支持通过 `for` 循环对**响应结果、统计信息、关键字段内容**进行实时访问，返回内容**实时累积**，支持按 `request_id` 或按索引访问：
批量流式调用中的累积幅度为 chunk by chunk；批量非流式调用中的累积幅度为 request by request；
...
for r in client.chat.batch(
    ...
    print(client.batch_result.results)    # 每个请求的标准响应，chunk by chunk 实时累积
...
批量 chat completions/Embeddings 调用都支持**进度回调、自定义请求 ID 、遇错停止**。
...
```python
{
    ...
    "choices": [{
        ...
        "message": {
            ...
            "reasoning_content": "推理过程内容..."    # 思考模型特有，非思考模型无此字段
        },
        "logprobs": null,
        "finish_reason": "stop"
    }],
...
# reasoning_content chunks (思考模型特有，非思考模型无此字段):
{'id': 'chatcmpl-xxx', 'choices': [{'index': 0, 'delta': {'reasoning_content': '推理..'}, 'finish_reason': None}]}
