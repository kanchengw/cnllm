...
CNLLM 致力于解决这一两难困境——通过提供一个调用中文大模型的**统一的接口**与一套**一致的参数规范**，在完整释放中文大模型原生能力的同时，将形态各异的响应自动转换为 OpenAI 标准格式。无论是 langchain、LlamaIndex 或其他机器学习框架，都能以相同的方式接入各类大模型；另外，在需要多模型协作的场景下，也能保持一致的接口和参数规范。
...
| `stop`              | str/list    | -  | -        |    ✅    |    ✅    | 停止序列                            |
| `user`              | str         | -  | -        |    ✅    |    ✅    | 用户标识                            |
| `response_format`   | dict        | -  | {type:"text"} |    ✅    |    ✅    | 响应格式                            |
