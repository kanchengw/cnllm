...
```python
{
    "status": {"elapsed": "3.42s", "success_count": 1, "fail_count": 1, "total": 2},  # 统计信息
    "usage": {"prompt_tokens": 5, "total_tokens": 5},  # 批处理的总用量信息
    "errors": {"request_1": "error message"},  # 所有失败请求的 request_id 和错误信息映射
    "results": {
        "request_0": [chunk1, chunk2, chunk3],  # 所有成功请求的 request_id 和标准响应映射
    },
...
| **errors**  | 错误请求的信息     | `resp.errors` / `batch_result.errors`         | `Dict[str, str]`             | `{"request_1": "error message"}`                                        |
...
| 严格模式     | `drop_params="strict"` | 抛出 `InvalidRequestError`，请求终止 |
